(function ($) {
    ...
    $.fn.btComboBox = function (arg) {
        ...
        $(this).each(function (i, e) {
            ...
            var attr_id = s.attr("id"),
                attr_value = s.attr("value"),
                attr_btn_class = s.attr("data-btn-class"),
                xbc = (attr_btn_class && attr_btn_class != "") ? ' ' + attr_btn_class : "",
                button = $('<button type="button" class="btn' + xbc + ' dropdown-toggle" data-toggle="dropdown"><span class="label custom_label"></span><span class="caret"></span></button>').attr("name", s.attr("name")).attr("id", attr_id + "-tog"),
                label = $(".label", button),
                ul = $('<ul class="dropdown-menu" role="menu"></ul>');

            $("option", s).each(function (j, f) {
                var o = $(f);
                $('<li><a href="' + o.val() + '">' + o.text() + '</a></li>').appendTo(ul)
            });

            $('<input type="hidden" name="' + s.attr("name") +
            '" id="' + attr_id + '-hid" value="' + ((attr_value) ? attr_value : "") + '" />').
                insertAfter(s);

            var sc = s.attr("class"), xc = (sc && sc != "") ? ' ' + sc : "",
                g = $('<div class="btn-group ' + xc + '"></div>').
                    attr("id", attr_id).append(button).append(ul);

            if (!select(g, attr_value) && !attr_value) {
                var so = selectedOption(g);
                if (so) selectValue(attr_id, g, button, so[2]);
            }

            $("li > a", g).click(function () {
                var a = $(this), ret = selectValue(attr_id, g, button, a);
